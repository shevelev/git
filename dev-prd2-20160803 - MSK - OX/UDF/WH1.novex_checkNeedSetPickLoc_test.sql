...
-- =============================================
ALTER FUNCTION [WH1].[novex_checkNeedSetPickLoc_test] 
...
(s.putawayzone in		-- обрабатываем только зоны в которых есть разделение на штучный и коробочный отбор
 ...
 and									-- обрабатываем только зоны стеллажного хранения
 (select top 1 loc from loc l where l.putawayzone=left(s.putawayzone,len(s.putawayzone)-2)+'EA') like '1___._._'
 and									-- исключаем зоны напольных ячеек
 (select top 1 isnull(loc,'1xxx.0.0') from loc l where l.putawayzone=left(s.putawayzone,len(s.putawayzone)-2)+'EA') not like '1___.0.0'
