...
ALTER PROCEDURE [dbo].[rep07F_PickListByCARTONGROUP](
	@wh varchar(30)=null,
...
--select * from wh40.pickdetail where orderkey = '0000000670'
		...
		select ORDERKEY,  CASEID,dbo.GETEAN128(CASEID) as BC_CASEID,COMPANY, REQUESTEDSHIPDATE, FROMLOC, oDOOR DOOR,externorderkey,
				PDOOR, ORDERDATE,sum( QTY/CASECNT) RES, count(distinct Caseid) pcnt,
				ClientName,
				CARTONGROUP, DROPID, TRANSPORTATIONSERVICE, dbo.GETEAN128(TRANSPORTATIONSERVICE) as BC_TRANS, SUSR4
		from #tmpt
