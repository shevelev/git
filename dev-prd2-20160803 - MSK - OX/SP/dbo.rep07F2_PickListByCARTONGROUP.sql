...
ALTER PROCEDURE [dbo].[rep07F2_PickListByCARTONGROUP](
	@wh varchar(30)=null,
...
--				max(PDOOR) PDOOR, max(ORDERDATE) ORDERDATE,sum( QTY/CASECNT) RES, count(distinct Caseid) pcnt,
--				max(ClientName) ClientName,
--				max(CARTONGROUP) CARTONGROUP, 
--				max(DROPID) DROPID, max(TRANSPORTATIONSERVICE) TRANSPORTATIONSERVICE, 
--				max(SUSR4) SUSR4,
--				max(SUSR2) SUSR2,
--				max(C_CITY) C_CITY,
...
select distinct CASEID CASEID, ORDERKEY,  dbo.GETEAN128(ORDERKEY) as BC_ORDERKEY, dbo.GETEAN128(CASEID) as BC_CASEID,
				COMPANY COMPANY, REQUESTEDSHIPDATE REQUESTEDSHIPDATE, 
				FROMLOC FROMLOC, oDOOR DOOR, externorderkey externorderkey,
				PDOOR PDOOR, ORDERDATE ORDERDATE,sum( QTY/CASECNT) RES, count(distinct Caseid) pcnt,
				ClientName ClientName,
				CARTONGROUP CARTONGROUP, 
				DROPID DROPID, TRANSPORTATIONSERVICE TRANSPORTATIONSERVICE, 
				SUSR4 SUSR4,
				SUSR2 SUSR2,
				C_CITY C_CITY,
