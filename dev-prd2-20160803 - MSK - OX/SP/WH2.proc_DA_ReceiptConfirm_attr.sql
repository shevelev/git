ALTER PROCEDURE [WH2].[proc_DA_ReceiptConfirm_attr]
...
from wh2.RECEIPTDETAIL rd right join #lot6naedit la
...
from wh2.LOTATTRIBUTE lat right join #lot6naedit la
on lat.SKU=la.SKU
...
into #lot6edit
from
	    WH2.RECEIPTDETAIL rd right join
	    (SELECT r.RECEIPTKEY, r.SKU, r.LOTTABLE06, count(distinct r.LOTTABLE04+r.LOTTABLE05) count_lot
	    FROM [PRD2].[WH2].[RECEIPTDETAIL] r
		    left join wh2.RECEIPT rr on rr.RECEIPTKEY=r.RECEIPTKEY
		    left join wh2.po pd
...
from  WH2.RECEIPTDETAIL RD1 RIGHT JOIN 
	#lot6edit x
ON
...
from
    wh2.lotattribute la right join 
