...
*/
ALTER PROCEDURE [rep].[mof_Warehouse_performance]
...
begin
	...
	from wh2.PODETAIL p with (NOLOCK,NOWAIT)
	...
	from wh2.PODETAIL p with (NOLOCK,NOWAIT)
		join wh2.SKU s on s.STORERKEY = p.STORERKEY and s.SKU = p.SKU
	where EXTERNPOKEY='' and EXTERNLINENO='' 
	--p.WHSEID = 'dbo'
		and ( @date_from is NULL or p.EDITDATE >= @date_from )
		and ( @date_to is NULL or p.EDITDATE < @date_to )
	...
	from wh2.ITRN i with (NOLOCK,NOWAIT)
		join wh2.LOC l on i.FROMLOC = l.LOC
	...
	--from wh2.PICKDETAIL p with (NOLOCK,NOWAIT)
	--	join wh2.ITRN i on i.SOURCEKEY = p.PICKDETAILKEY and i.TRANTYPE = 'MV'
	...
	--	and ( @date_to is NULL or i.EDITDATE < @date_to )
		...
		select distinct dbo.udf_get_date_from_datetime(i.EDITDATE), 4 as COLUMN_ID, 'Комплектация' as HDR1, 'Заказы' as HDR2, p.ORDERKEY as ID, 0 as STDCUBE
	from wh2.ITRN i with (NOLOCK,NOWAIT)
		join wh2.PICKDETAIL p on i.SOURCEKEY = p.PICKDETAILKEY and i.TRANTYPE = 'MV'
	where p.[STATUS] >= '5' -- Picked
		and ( @date_from is NULL or i.EDITDATE >= @date_from )
		and ( @date_to is NULL or i.EDITDATE < @date_to )
	...
	from wh2.PICKCONTROL pc  with (NOLOCK,NOWAIT)
		join wh2.PICKDETAIL pd on pd.CASEID = pc.CASEID and pd.LOT = pc.LOT
	where pc.[STATUS] = 1
	...
	from wh2.ORDERDETAIL o with (NOLOCK,NOWAIT)
	...
	from wh2.ORDERDETAIL o with (NOLOCK,NOWAIT)
		join wh2.SKU s on s.SKU = o.SKU and s.STORERKEY = o.STORERKEY
