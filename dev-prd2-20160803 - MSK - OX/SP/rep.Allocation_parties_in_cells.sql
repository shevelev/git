ALTER PROCEDURE [rep].[Allocation_parties_in_cells] (
...
select lli.lot, loc, ID, lli.status,
	...
	from WH1.lotxlocxid lli
		left join WH1.LOTATTRIBUTE la on la.lot=lli.lot
		left join WH1.receipt r on la.lottable06=r.receiptkey
		left join WH1.storer s on s.storerkey=r.carrierkey
			 join WH1.storer s2 on s2.storerkey=lli.storerkey
			 join WH1.sku sk on sk.sku = lli.sku and sk.storerkey=lli.storerkey		
	where 1=1 ' +
