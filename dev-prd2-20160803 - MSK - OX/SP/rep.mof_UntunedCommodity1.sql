ALTER PROCEDURE [rep].[mof_UntunedCommodity1] (@wh varchar(30), @rc varchar(20) = null)
...
--select @rc='', @wh='wh2'
	...
	select s.SUSR10 susr10, s.sku, s.descr, s.storerkey,st.company,
		...
		into #sku
		from wh2.sku as s
		  left join (select distinct SKU, case when isnull(ALTSKU,'0')='0' then '0' else '1' end ALTSKU from wh2.ALTSKU) asku on s.SKU=asku.SKU
			left join wh2.receiptdetail as r on (r.sku = s.sku) and (r.storerkey = s.storerkey)
			join wh2.storer as st on (s.storerkey = st.storerkey) 
		where 1=2
