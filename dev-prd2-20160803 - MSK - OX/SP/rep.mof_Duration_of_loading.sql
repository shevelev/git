ALTER PROCEDURE [rep].[mof_Duration_of_loading]
...
--		l.DEPARTURETIME,
	...
	from ( select distinct * from DROPS where DROPID like 'TS%' ) d
		join wh2.PICKDETAIL p with (NOLOCK,NOWAIT)
			join wh2.ORDERS o with (NOLOCK,NOWAIT)
				...
				join (
					...
					from wh2.ORDERSTATUSHISTORY with (NOLOCK,NOWAIT)
...
--		d.DEPARTURETIME,
	...
	from #TS d
		join (
			...
			from wh2.DROPIDDETAIL with (NOLOCK,NOWAIT)
...
--		d.DEPARTURETIME,
	...
	from #RESULT r
		...
		join wh2.CODELKUP c with (NOLOCK,NOWAIT) on r.[STATUS] = c.CODE and c.LISTNAME = 'LOADSTATUS'
	order by DROPID
