...
ALTER PROCEDURE [WH2].[proc_DA_FizOstatki]
	@wh varchar(30),
...
--select  STORERKEY, SKU,  max(ALTSKU) altsku  into #altsku from WH2.ALTSKU group by STORERKEY, SKU
select  STORERKEY, SKU,  max(ALTSKU) altsku  into #altsku from WH2.[ALTSKU] group by STORERKEY, SKU
...
drop table #altsku
  ...
  BEGIN
	...
	from    [spb-sql1201].[DAX2009_1].[dbo].SZ_ImpInventsumFromWMS


	insert into [spb-sql1201].[DAX2009_1].[dbo].SZ_ImpInventsumFromWMS
