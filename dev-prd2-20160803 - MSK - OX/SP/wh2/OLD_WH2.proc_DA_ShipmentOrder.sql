...
ALTER PROCEDURE [WH2].[proc_DA_ShipmentOrder]
	@id int
as  
...
set @loadid = ''
select @deadtime = short from WH2.CODELKUP where LISTNAME = 'sysvar'and code = 'DLINESH'
			...
			select 
				@msg_errdetails1 = @msg_errdetails1 --externorderkey empty
					+case when o.externorderkey = ''
						then 'ORDER. ВнешнийНомер=Пустой.'+@enter
						else ''
					end,				
				@msg_errdetails1 = @msg_errdetails1 --storerkey EMPTY
					+case when o.storerkey = ''
						then 'ORDER. ВнешнийНомер='+o.externorderkey+'. Владелец=пустой.'+@enter
						else ''
					end,
				@msg_errdetails1 = @msg_errdetails1 --storerkey in STORER
					+case when (not exists(select s.* from WH2.storer s where s.storerkey = o.storerkey))
						then 'ORDER. ВнешнийНомер='+o.externorderkey+'. Владелец='+o.storerkey+' отсутвует в справочнике STORER.'+@enter
						else ''
					end,
				@msg_errdetails1 = @msg_errdetails1 --c_contact1 EMPTY
					+case when o.c_contact1 = ''
						then 'ORDER. Накладная=пустая.'+@enter
						else ''
					end,
				--@msg_errdetails1 = @msg_errdetails1 --carriercode in STORER
				--	+case when (not exists(select s.* from WH2.storer s where s.storerkey = o.carriercode))
				--		then 'ORDER. CARRIERcode='+o.carriercode+' отсутвует в справочнике STORER.'+@enter
				--		else ''
				--	end,
				@msg_errdetails1 = @msg_errdetails1 --type empty
					+case when o.type = ''
						then 'ORDER. ВнешнийНомер='+o.externorderkey+'. ТипДокумента=пустой.'+@enter
						else ''
					end,
				...
				@msg_errdetails1 = @msg_errdetails1 --SUSR1 empty
					+case when o.susr1 = ''
						then 'ORDER. ВнешнийНомер='+o.externorderkey+'. СкладОтгрузки=пустой'+@enter
						else ''
					end,
				@msg_errdetails1 = @msg_errdetails1 --SUSR2 empty
					+case when o.susr2 = ''
						then 'ORDER. ВнешнийНомер='+o.externorderkey+'. SUSR2=empty.'+@enter
						else ''
					end,
				@msg_errdetails1 = @msg_errdetails1 --SUSR4 empty
					+case when o.susr4 = ''
						then 'ORDER. ВнешнийНомер='+o.externorderkey+'. Маршрут(SUSR4)=пустой.'+@enter
						else ''
					end,
				@msg_errdetails1 = @msg_errdetails1 --routedeleted empty
					+case when o.routedeleted = ''
						then 'ORDER. ВнешнийНомер='+o.externorderkey+'. routedeleted=empty.'+@enter
						else ''
					end,
				--@msg_errdetails1 = @msg_errdetails1 --SUSR4 empty
				...
				@msg_errdetails1 = @msg_errdetails1 --consigneekey in STORER
					+ 
					case when (not exists(select s.* from WH2.storer s where s.storerkey = o.consigneekey)) 
						then 'ORDER. ВнешнийНомер='+o.externorderkey+'. КлиентДоставки='+o.consigneekey+' отсутвует в справочнике STORER.'+@enter
						else ''						
					end,
				@msg_errdetails1 = @msg_errdetails1
					+case when oo.externorderkey is not null and cast(oo.STATUS as int) > 9 
						then 'ORDER. ВнешнийНомер='+oo.externorderkey+' SUSR2= ' + oo.susr2 +'. Документ в обработке, обновление невозможно.'+@enter
						else ''
					end--,
				--@msg_errdetails1 = @msg_errdetails1 --consigneekey in STORER
				...
				--		then ''
				--		else 
				--			case when (not exists(select s.* from WH2.storer s where s.storerkey = o.b_company))
				...
				begin
				    ...
				    select 
					    ...
					    @msg_errdetails1 = @msg_errdetails1 --storer in STORER
						    +case when (not exists(select s.* from WH2.storer s where s.storerkey = od.storerkey))
					    ...
					    @msg_errdetails1 = @msg_errdetails1 --storer+sku in SKU
						    +case when (not exists(select s.* from WH2.sku s where s.storerkey = od.storerkey and s.sku = od.sku))
					    ...
					    @msg_errdetails1 = @msg_errdetails1 --qtyexpected
						    +case when (od.qtyexpected <= cast(0 as numeric(22,5)))
							    then 'ORDER. EXTERNORDERkey='+od.externorderkey+' SKU='+od.sku+'. Не корректное значение qtyexpected.'+@enter
							    else ''
						    end					    
...
--							
				...
				begin
					...
					begin
					    ...
					    exec dbo.DA_GetNewKey 'WH2','order',@orderkey output

					    print ' вставляем шапку документа'
					    
					    insert into WH2.orders 
					    ...
					    select  top (1) 'WH2' whseid, @orderkey, o.storerkey, o.externorderkey, o.[type], 
					    ...
					    BEGIN
						    ...
						    select  'WH2' as whseid,@orderkey,
							    ...
							    s.packkey,dod.LOTTABLE02,dod.LOTTABLE04,dod.LOTTABLE05,left(dod.LOTTABLE06,25) as LOTTABLE06,	dod.LOTTABLE06 as susr1,				   
						    ...
						    from    #DA_ShipmentOrderDetail dod							
							    join WH2.sku s 						on s.sku = dod.sku and s.storerkey = dod.storerkey
							    join WH2.strategy st 				on s.strategykey = st.strategykey 
							    join WH2.allocatestrategy ast 		on ast.allocatestrategykey = st.allocatestrategykey
						    ...
						    begin
							    ...
							    --set @loaddate = convert(datetime,convert(char(10),@shipdate,101))
							    ...
							    begin --создаем загрузку
								    ...
								    exec dbo.DA_GetNewKey 'WH2','CARTONID',@loadid output
								    ...
								    select 'WH2',@loadid,@carriercode, @susr4, '0',@loaddate, 'VOROTA2'
								    ...
								    BEGIN
									    ...
									    exec dbo.DA_GetNewKey 'WH2','LOADSTOPID',@loadstopid output
        								    
									    print '6.1.2. добавляем СТОП в загрузку'
        								    
									    insert WH2.loadstop (whseid,loadid,loadstopid,[stop],[status])
									    
									    select  'WH2',@loadid,@loadstopid,1,'0'											
							    ...
							    exec dbo.DA_GetNewKey 'WH2','LOADORDERDETAILID',@loadorderdetailid output
        								
							    insert WH2.loadorderdetail 
							    (whseid,loadstopid,loadorderdetailid,storer, 
							    shipmentorderid, customer, OHTYPE)
        							
							    select 'WH2',@loadstopid,@loadorderdetailid,@storerkey,@orderkey,@consigneekey, '1'
