...
create table #table_Paket (
		Storer varchar(15) not null, -- Владелец
		Company varchar(45) not null, -- Название владельца
		SummaPak decimal(22,2) not null -- Кол-во пакетов документов		
)
...
where ord.DELIVERYDATE2 between '''+@bdate+''' and  '''+@edate+'''
		and (ord.status=''92'' or ord.status=''95'')
		and (ord.susr2 not like ''Внутренее перемещение''
				and ord.susr2 not like ''Возврат брака''
				and ord.susr2 not like ''Возврат товара поставщику''
				and ord.susr2 not like ''Инвентаризация (недостача)''
				and ord.susr2 not like ''Недопоставка товара''
				and ord.susr2 not like ''Реализация 2-ой тип%''
				and ord.susr2 not like ''Списание''
				and ord.susr2 not like ''Штрафные санкции'')
group by  storer.storerkey, storer.company
'
--'+case when @tekdate=0 
--					then 'ord.DELIVERYDATE2 between '''+@bdate+''' and  '''+@edate+''''
...
select  tS.Storerkey Storer,
		...
		--null Arrival,
		sum(fto.Qty) Total,
		cast(cast(300/@kolday as decimal(22,2)) as varchar(10))+' руб.' Tariff,
...
/*Заполнение полей для Ренесанскосметик*/
insert into #table_Storer 
