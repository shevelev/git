ALTER PROCEDURE [rep].[mof_Performance_and_errors_of_order_picker]
...
begin
	...
	----------------------------------------------------------
		...
		from wh2.PICKCONTROL pc with (NOLOCK,NOWAIT)
			join (select distinct pd.ORDERKEY, pd.SKU, pd.CASEID, pd.LOT from wh2.PICKDETAIL pd with (NOLOCK,NOWAIT) 
				join wh2.ITRN i on i.SOURCEKEY = pd.PICKDETAILKEY
...
----------------------------------------------------------		
	...
	select
		...
		count(*) as ROWS_CNT
	from wh2.ITRN i with (NOLOCK,NOWAIT)
		join wh2.LOC fl with (NOLOCK,NOWAIT) on i.FROMLOC = fl.LOC
		join wh2.LOC tl with (NOLOCK,NOWAIT) on i.TOLOC = tl.LOC
	...
	select
		...
		count(*) as ROWS_CNT
	from wh2.ITRN i with (NOLOCK,NOWAIT)
		join wh2.LOC l with (NOLOCK,NOWAIT) on i.TOLOC = l.LOC
		join ssaadmin.pl_usr pu with (NOLOCK,NOWAIT) on i.EDITWHO = pu.usr_login
	where i.TRANTYPE = 'MV'
