ALTER PROCEDURE [dbo].[check_docdetail] (
	@dockey varchar (15),
...
-- определение системы документа
			...
			set @sql =
			'insert into #result
			SELECT r_linenumber,r_articul,r_descr,sm_qty,sm_unitprice,sm_unitprice_1,sm_nds,sm_summs,sm_summs_1,0,0,0,0,'''',client_index,'''',name_clients,''''
				FROM OPENROWSET(''MSDASQL'',''DSN=SMarket;UID=SYSINFOR;PWD=3'',
					...
					cs.name r_descr, 
					ds.quantity sm_qty, 
					round(ds.pricerub,10) sm_unitprice, 
					ds.pricerub_1 sm_unitprice_1, 
					case when ds.nds = 0 or ds.pricerub = 0 then 0 else round(ds.nds/((ds.quantity*ds.pricerub-ds.nds)/100),10) end sm_nds, 
					round(ds.quantity*ds.pricerub,10) sm_summs, 
...
print @sql
			...
			select
				...
				case when p.status >= '11' then pd.qtyreceived else pd.qtyadjusted end,
				pd.unitprice,
				pd.unit_cost,
				case when p.status >= '11' then pd.unitprice*pd.qtyreceived else pd.unitprice*pd.qtyadjusted end,
				p.sellersreference,'',st.company,'',''
...
-- определение системы документа
			...
			select
				...
				o.b_company, '', st.company, '', o.susr5
		...
		begin
			...
			if @system = 0
				begin
					...
					set @sql =
					'insert into #result
					SELECT r_linenumber,r_articul,r_descr,sm_qty,sm_unitprice,sm_unitprice_1,sm_nds,sm_summs,sm_summs_1,0,0,0,0,'''','''','''','''',''''
						FROM OPENROWSET(''MSDASQL'',''DSN=SMarket;UID=SYSINFOR;PWD=3'',
							''select 
							ds.pos_number r_linenumber, 
							ds.articul r_articul,
...
--select * from #result_iwm
					update r set r.wm_qty = ri.qty, r.wm_unitprice = 0, r.wm_nds = 0, r.wm_summ = 0
...
--print @sql select * from #idr
			...
			insert into #result_r
				...
				wm_susr5)
			select
				'',i.sku,s.descr,0,0,0,0,0,0,i.qty,0,0,0,'','','','',''
