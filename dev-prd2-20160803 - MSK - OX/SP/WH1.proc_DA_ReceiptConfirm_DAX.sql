...
ALTER PROCEDURE [WH1].[proc_DA_ReceiptConfirm_DAX]
	@wh varchar(30),
...
--
/*
 select * from wh1.podetail where pokey in (select pokey from wh1.po where otherreference = '0000045594')
 select * from wh1.receiptdetail where receiptkey = '0000045594'
-- delete from wh1.podetail where pokey in (select pokey from wh1.po where otherreference = '0000045594') and (ALTSKU != '' or altsku is null)
*/
...
while (exists(select * from #podetailresult))
	begin
		...
		insert into wh1.PODETAIL (WHSEID,pokey,	EXTERNPOKEY,POLINENUMBER,EXTERNLINENO,
...
--print 'выбираем номера ЗЗ для пуо'
	...
	select identity(int,1,1) idpo, pokey into #pokey from wh1.PO where OTHERREFERENCE = @receiptkey order by POKEY
		...
		select @potype=POTYPE from wh1.PO where POKEY=@pokey
		...
		if @potype not in ('4','5') 
			begin		
				...
				select  'SZ' DataAReaID,EXTERNPOKEY,POTYPE, '1' INFORDAXSYSTEM ,SELLERNAME,SUSR2,  '27' Status,'5' daxstatus, GETDATE() ReceivedDate,
   					@n + tp.idpo as recid
				from    #pokey tp
				join wh1.PO p on tp.POKEY = p.POKEY
...
print '9.1. обновление susr5 в переданном ПУО'
	...
	update WH1.TRANSMITLOG 	set KEY5 = '1'	where TRANSMITLOGKEY = @transmitlogkey	
