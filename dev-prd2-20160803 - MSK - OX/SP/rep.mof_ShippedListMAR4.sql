/* Погрузочный лист */
ALTER PROCEDURE [rep].[mof_ShippedListMAR4] (
	@dat1 datetime,
	@dat2 datetime,
	@mar varchar(12)
)
...
exec dbo.DA_GetNewKey 'wh2','TS',@ts output
...
from	wh2.loadhdr lh
join wh2.loadstop ls on lh.LOADID=ls.LOADID
join wh2.LOADORDERDETAIL lod on ls.LOADSTOPID=lod.LOADSTOPID
...
update C set loci = i.fromloc
	from #case c left join wh2.ITRN i on i.SOURCEKEY = c.pickdetailkey
	where TRANTYPE = 'MV'
...
update C set c.zone = pz.PUTAWAYZONE, c.control = pz.CARTONIZEAREA
	from #case c join wh2.LOC l on c.loc = l.LOC
...
update C set c.status = isnull(p.status,0)
	from #case c left join wh2.pickcontrol p on c.caseid = p.caseid and p.RUN_ALLOCATION='0' and p.RUN_CC='0'
...
into #case5
from #case1 c1
join wh2.PICKCONTROL pc on c1.caseid=pc.CASEID
...
into #t3
from #case1 c1
join wh2.PICKDETAIL pd on c1.caseid=pd.CASEID and c1.orderkey=pd.ORDERKEY
...
CASE when isnull(t3.BOXNUM,'')='' then t3.yashik else t3.BOXNUM end as tt, c4.DEPARTURETIME, c4.loadid
 from #case4 c4
join wh2.pickdetail pd on c4.orderkey=pd.ORDERKEY
left join #t3 t3 on t3.CASEID=pd.CASEID
join wh2.DROPID d on d.DROPID=pd.dropid
join wh2.CODELKUP ck on pd.STATUS=ck.CODE and LISTNAME='ordrstatus'
join wh2.ORDERS o on pd.orderkey=o.orderkey
join wh2.ORDERSTATUSSETUP oss on o.STATUS=oss.CODE
join wh2.storer s on o.CONSIGNEEKEY=s.storerkey
