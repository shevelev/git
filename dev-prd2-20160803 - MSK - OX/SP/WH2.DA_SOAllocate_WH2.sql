ALTER PROCEDURE [WH2].[DA_SOAllocate_WH2] --(
	--@wh varchar(10)	= NULL
...
begin	
	...
	SELECT 'Soallocate-wh2', GETDATE()
	
	print 'выбираем заказы, которые не запущены и не зарезервированы'
	select o.*, lh.DEPARTURETIME 
	into #orders 
	from wh2.orders o 
		join wh2.LOADORDERDETAIL lo on o.ORDERKEY = lo.SHIPMENTORDERID
		join wh2.LOADSTOP ls on lo.LOADSTOPID = ls.LOADSTOPID
	...
	update wh2.orders set susr5 = @status_run where status > '09' and susr5 != '' and EDITDATE between dateadd(hh,-24, @departTime) and getdate()
...
--select SUSR5,* from #orders where susr5 in ( @status_wait, @status_repl)
			...
			-- старое условие: (select COUNT(*) from wh2.ORDERS where ORDERKEY = @orderkey and dateadd(n,3,EDITDATE) <= getdate()) != 0
			begin
				update wh2.orders set susr5 = @status_timeout /*а вот тут трабла - если заказ был проверен до истечения таймаута, то он может зависнуть тут надолго. убрано /--, EDITDATE = GETDATE()--/ */ 
		...
		begin -- заказов, ожидающих резервирования - нет.
			...
			begin
				...
				update o set o.SUSR5 = '' , EDITDATE = getdate()
					from wh2.ORDERS o join #orders oo on o.ORDERKEY = oo.orderkey
...
--		end -- while (@doNext and exists......
	...
	begin
		...
		update wh2.orders set susr5 = @status_noorder, EDITDATE = getdate() where orderkey = @orderkey
...
--	if exists (select 1 from #orderdetail where susr1 not in ('СкладПродаж','СД') or storerkey not in ('001'))
	if exists (select 1 from #orderdetail where susr1 not in ('Москва','МС_СД'))

	begin
		print 'Заказ сразу же резервируем ' + @orderkey		
		print 'все товары есть, выдаем номер заказа и меням статус'
		
		update wh2.orders set susr5 = @status_wait, EDITDATE = getdate() where orderkey = @orderkey
		
		insert into dbo.SOAllocateLog (orderkey, susr5) 
		select @orderkey, @status_wait
		
		select orderkey, externorderkey from wh2.orders where orderkey = @orderkey
	...
	begin
		...
		--into #skuqty
		from wh2.LOTXLOCXID lli 
			join wh2.LOTATTRIBUTE la on lli.LOT = la.LOT
			...
			join wh2.loc l on l.loc = lli.loc
		--!!!!!!!!!!!!!!!left join wh2.INVENTORYHOLD ih on ih.loc = lli.LOC or ih.LOT = lli.LOT or ih.ID = lli.id
		where lli.QTY > 0 
		...
		begin
			...
			update wh2.orders set susr5 = @status_nosku, EDITDATE = getdate() where orderkey = @orderkey 
	...
	truncate table #qtycase
		...
		begin
			...
			from	#skuqty sq 
				...
				--left join wh2.PACK p on p.PACKKEY = sq.lottable01
	...
	--print 'общее свободное количество товара на остатках склада штуки.'		
			...
			begin
				...
				begin
					...
					update wh2.orders set susr5 = @status_repl, EDITDATE = dateadd(n, 20, getdate())  where orderkey = @orderkey -- Шевелев 14.09.2015
					--update wh2.orders set susr5 = @status_repl, EDITDATE = getdate() where orderkey = @orderkey
	...
	begin
		...
		select 1 from wh2.orders where orderkey = @orderkey1
	end
