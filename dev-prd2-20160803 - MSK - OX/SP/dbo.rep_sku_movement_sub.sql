CREATE proc [dbo].[rep_sku_movement_sub]
...
begin
	...
	select distinct l.SKU, l.LOT
		...
		from wh1.LOTATTRIBUTE l
			...
			join wh1.itrn i on i.SKU=l.sku and i.LOT=l.lot
		where	( @sku is NULL or l.SKU = @sku )
			and ( @serial is NULL or l.LOTTABLE02 = @serial )
			and ( @description is NULL or s.DESCR like '%' + replace(@description,' ','%') + '%' )
			and ( @date_from  is NULL or i.ADDDATE > @date_from)
			and ( @date_to is NULL or i.ADDDATE < @date_to)
			
			
		select i.SKU, s.DESCR, i.LOT, la.LOTTABLE02 as SERIAL, la.LOTTABLE05 as EXP_DATE, SUM(qty) as ACCEPTED_QTY
		 into #lots_current
		from #lots l
			join wh1.itrn i on i.LOT=l.LOT
			join wh1.LOTATTRIBUTE la on l.LOT=la.LOT
			join wh1.sku s on l.SKU=s.sku
		where i.TRANTYPE in ('DP','WD','AJ')
				and ( @date_from  is NULL or i.ADDDATE > @date_from)
				and ( @date_to is NULL or i.ADDDATE < @date_to)
		group by i.SKU, s.DESCR,i.LOT, la.LOTTABLE02, la.LOTTABLE05
			
		select l.* , lot.QTY CUR_QTY
			from #lots_current l
			join wh1.lot lot on l.LOT=lot.LOT
			
		drop table #lots_current
		drop table #lots
