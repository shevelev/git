...
from	wh1.TRANSMITLOG t
	join wh1.itrn i 
	    on t.key1 = i.itrnkey
	join wh1.LOTATTRIBUTE la
	    on la.LOT = i.LOT
	join #lo l 
	    on l.loc = i.toloc 
	join #lo l2 
	    on l2.loc = i.fromloc 
	left join dbo.WHTOZONE w 
	    on w.zone = l.putawayzone 
	left join dbo.WHTOZONE w2 
...
where	t.TRANSMITLOGKEY = @transmitlogkey
	...
	from #q r
		join wh1.ITRN i on r.ITRNKEY=i.ITRNKEY
...
from    [spb-sql1201].[DAX2009_1].[dbo].SZ_ImpInventjournal


insert into [spb-sql1201].[DAX2009_1].[dbo].SZ_ImpInventjournal
...
begin	    
    ...
    select  @n = isnull(max(cast(cast(recid as numeric) as bigint)),0)
    from    [spb-sql1201].[DAX2009_1].[dbo].SZ_ImpInventjournaltrans


    insert into [spb-sql1201].[DAX2009_1].[dbo].SZ_ImpInventjournaltrans
