...
ALTER PROCEDURE [WH1].[proc_DA_ReceiptConfirm]
	@wh varchar(30),
...
EXEC	@return_value1 = [dbo].[proc_DA_ReceiptConfirm_attr]
...
-------------------------------
    ...
    select  p.POKEY, p.POTYPE,p.EXTERNPOKEY,pd.EXTERNLINENO,
	    ...
	    pd.lottable01,pd.LOTTABLE02,pd.lottable04,pd.lottable05,pd.lottable06, pd.RETAILSKU -- 14.09.2016 _A%%
    from    wh1.PO p
	    join wh1.PODETAIL pd
		    on pd.POKEY = p.POKEY
    ...
    begin	
	    ...
	    --begin			
		    ...
		    select  q.id,pd.POKEY, pd.POTYPE,pd.EXTERNPOKEY,pd.externlinenum,pd.STORERKEY,pd.SKU,pd.qtyordered,
			    q.descr,q.PACKKEY,q.UOM,q.ALTSKU, q.LOTTABLE01, q.LOTTABLE02, q.LOTTABLE04, q.LOTTABLE05, q.LOTTABLE06, pd.retailsku,  --- _A%%
    ...
    begin				
	   ...
	   insert into #pr_it		
	    select  r.id,'' as POKEY, '' as POTYPE,'' as EXTERNPOKEY,pd.externlinenum,r.STORERKEY,r.SKU,0 as qtyordered,
		    r.descr,r.PACKKEY,r.UOM,r.ALTSKU, r.LOTTABLE01, r.LOTTABLE02, r.LOTTABLE04, r.LOTTABLE05, r.LOTTABLE06, pd.retailsku,  --- _A%%
	    ...
	    insert into #pr_it		
	    select  r.id,'' as POKEY, '' as POTYPE,'' as EXTERNPOKEY,r.id+1000 as externlinenum,r.STORERKEY,r.SKU,0 as qtyordered,
		    r.descr,r.PACKKEY,r.UOM,r.ALTSKU, r.LOTTABLE01, r.LOTTABLE02, r.LOTTABLE04, r.LOTTABLE05, r.LOTTABLE06, pd.retailsku,  --- _A%%
    ...
    begin tran
    print 'вставка результатов в ЗЗ'
    ...
    begin			
	    ...
	    lottable01, LOTTABLE02, lottable04, lottable05, lottable06,RETAILSKU,ADDWHO)
    	
    	
	    select  'WH1',pokey,externpokey,
		    ...
		    lottable01, LOTTABLE02, lottable04, lottable05, lottable06, retailsku ,'asnclosed' -- _A%%
	    from    #pr_it
	    where   id = @poid
    ...
    begin
	    ...
	    begin
		    ...
		    select  'WH1',n.pokey,externpokey,
			    REPLICATE('0',5 - LEN(@polinenumber+1)) + CAST(@polinenumber+1 as varchar(10)) as polinenumber,
			    n.EXTERNLINENO,
...
--			    n.lottable01, n.LOTTABLE02, n.lottable04, n.lottable05, n.lottable06,'asnclosed'
    ...
    begin
	    ...
	    select  pd.POKEY,pd.EXTERNPOKEY,pd.EXTERNLINENO,pd.STORERKEY,pd.SKU, pd.QTYORDERED,
		    --sum(pd.QTYRECEIVED) as QTYRECEIVED,
		    pd.QTYRECEIVED,
		    pd.lottable01, pd.LOTTABLE02, pd.lottable04, pd.lottable05, pd.lottable06, pd.RETAILSKU,
		    pd.SUSR4,
	    ...
	    select  IDENTITY(int,1,1) as id,
		    n.POKEY,n.EXTERNPOKEY,n.EXTERNLINENO,n.STORERKEY,n.SKU,n.QTYORDERED,nn.QTYRECEIVED,
		    nn.lottable01, nn.LOTTABLE02, nn.lottable04, nn.lottable05, nn.lottable06, nn.RETAILSKU
	    into    #ned4
	    from    (select pokey,EXTERNPOKEY,EXTERNLINENO,STORERKEY,SKU,sum(QTYORDERED) as qtyordered,
	    ...
	    select  IDENTITY(int,1,1) as id,
		    n.POKEY,n.EXTERNPOKEY,n.EXTERNLINENO,n.STORERKEY,n.SKU,n.QTYORDERED,nn.QTYRECEIVED,
		    n.lottable01, n.LOTTABLE02, n.lottable04, n.lottable05, n.lottable06, n.RETAILSKU
	    into    #ned5
	    from    (select pokey,EXTERNPOKEY,EXTERNLINENO,STORERKEY,SKU,sum(QTYORDERED) as qtyordered,
			    lottable01, LOTTABLE02, lottable04, lottable05, lottable06 , RETAILSKU
	             from   #ned3
    	...
    	*/
	    ...
	    begin
		    ...
		    select  'WH1',n.pokey,externpokey,
			    ...
			    n.QTYORDERED - n.QTYRECEIVED as qtyreceived,n.sku,LEFT(s.DESCR,60) as descr,n.storerkey,'LOSTPRIEM' as sclad,s.packkey,
			    s.RFDEFAULTUOM,IsNull(r.ALTSKU,s.ALTSKU) as ALTSKU,
			    n.lottable01, n.LOTTABLE02, n.lottable04, n.lottable05, n.lottable06, n.RETAILSKU,'asnclosed'
		    from    #ned4 n
			    left join (
			    	    select sku,storerkey,max(ALTSKU) as ALTSKU
			    	    from wh1.RECEIPTDETAIL 
		    ...
		    select @text = 'java -classpath c:\DA_Axap\DKInforDA.jar -DconfigPath=c:\DA_Axap\DKInforDA.properties dke.da.trident.ExceedServerCall Receipt PRD2_WH1 asnclosed '+ @receiptkey+',001,'+@tolot+','+@receiptkey+','+@sku+','+@pokey+','+@qty+','+@uom+','+@packkey+','+@toloc+','+@toid+',,N,,'+@lot01+','+@lot02+',,'+@lot04+','+@lot05+','+@lot06+',,,,,,,,,,0,,0,,,,,,'
		   
		    print 'анализ ЗЗ тип <> 0,1 - поставить на балансы'
		    insert into #t1 
		    exec xp_cmdshell @text
		    --exec xp_cmdshell 'java -classpath c:\dataadapter\DKInforDA.jar -DconfigPath=c:\dataadapter\DKInforDA.properties dke.da.trident.ExceedServerCall Receipt PRD2_WH1 asnclosed @receiptkey,@storerkey,@tolot,@receiptkey,@sku,,@qtyreceived,@uom,@packkey,@toloc,@toid,,N,,@lot01,@lot02,,@lot04,@lot05,@lot06,,,,,,,,,,0,,0,,,,,,'
		    ...
		    select  'WH1' as WHSEID,@receiptkey,
    ...
    ------------------------ Переход RETAILSKU в поле LOTTABLE06 14.09.2016 -----------------------
    update p2
	 set LOTTABLE06 = case when (LEN(RETAILSKU) > LEN(LOTTABLE06))
							then RETAILSKU
							else LOTTABLE06
						end
	from wh1.PODETAIL p2
		join wh1.po p on p2.POKEY = p.POKEY
	where p.OTHERREFERENCE = @receiptkey and LEN(RETAILSKU) > LEN(LOTTABLE06)
    ------------------------ Переход RETAILSKU в поле LOTTABLE06 14.09.2016 -----------------------
    ...
    select  p.EXTERNPOKEY, p.POTYPE, p.SELLERNAME, p.BUYERADDRESS4, p.SUSR2,
	    ...
	    p2.ALTSKU,
	    --p2.SUSR4,
	    case    when  p2.SUSR4 = 'GENERAL' then 'СкладПродаж'
	    ...
	    --p2.RETAILSKU LOTTABLE06,p2.ADDWHO  -- _A%%
    into    #itog	
    from    wh1.PO p
	    join wh1.PODETAIL p2
		on p2.POKEY = p.POKEY
    where   p.OTHERREFERENCE = @receiptkey


    select  identity(int,1,1) as id,
	    EXTERNPOKEY,POTYPE, SELLERNAME, BUYERADDRESS4, SUSR2, STORERKEY
    into    #ii		
    from    (select distinct EXTERNPOKEY,POTYPE, SELLERNAME, BUYERADDRESS4, SUSR2, STORERKEY from #itog)i
...
--update wh1.TRANSMITLOG set ADDWHO = 'proc_DA_ReceiptConfirm' where TRANSMITLOGKEY  = @transmitlogkey
