ALTER PROCEDURE [rep].[mof_unspent_balance] (
@loc1 varchar(15),
@loc2 varchar(15),
@u varchar(15),
@br varchar(15)
...
from wh2.lotxlocxid lli 
		left join wh2.lotAttribute la on lli.lot=la.lot
		left join wh2.sku s on lli.sku=s.sku and lli.storerkey=s.storerkey
...
while (select COUNT(*) from #d_sku) != 0
	begin
		...
		print @sku
		insert into #altsku2 select top (1)sku, altsku from wh2.ALTSKU where SKU=@sku
