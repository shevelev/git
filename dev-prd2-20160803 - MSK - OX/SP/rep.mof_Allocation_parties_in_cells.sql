ALTER PROCEDURE [rep].[mof_Allocation_parties_in_cells] (
...
select lli.lot, loc, ID, lli.status,
	...
	from wh2.lotxlocxid lli
		left join wh2.LOTATTRIBUTE la on la.lot=lli.lot
		left join wh2.receipt r on la.lottable06=r.receiptkey
		left join wh2.storer s on s.storerkey=r.carrierkey
			 join wh2.storer s2 on s2.storerkey=lli.storerkey
			 join wh2.sku sk on sk.sku = lli.sku and sk.storerkey=lli.storerkey		
	where 1=1 ' +
        ...
        case when isnull(@sku,'')=''  then '' else ' and  (sk.sku='''+@sku+''')' end +
		' and  (qty > 0)'
