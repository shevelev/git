ALTER PROCEDURE [dbo].[proc_DA_SKU333]
...
declare @altskuid varchar(max) -- заменяющий товар
...
BEGIN TRY
	...
	begin
		...
		set	ds.storerkey = case    when (left(isnull(rtrim(ltrim(storerkey)),''),15)) = 'SZ' then '001' 
			...
			ds.altskuid = left(isnull(ds.altskuid,''),10)
		...
		begin
			...
			if (exists (select ws.* from wh1.sku ws where ws.storerkey = @storerkey and ws.sku = @sku))
				begin
					...
					print ' обновляем WH1.sku='+@sku+', storer='+@storerkey
					...
					from	wh2.sku s 
							...
							join wh2.CODELKUP c
								on c.CODE = ds.FreightClass
								and c.LISTNAME = 'FREIGHTCLS'
							join wh2.strategyxsku dsg 
								on dsg.class = c.SHORT
					if @@rowcount = 0 
					begin
						set @msg_errdetails = @msg_errdetails+'WH2.SKU. sku='+@sku+', STORERkey='+@storerkey+'. Неудалось выполнить обновление сушествующей записи.'+char(10)+char(13)
						set @send_error = 1
					end
				...
				begin  
					...
					if @@rowcount = 0 
					begin
						set @msg_errdetails = @msg_errdetails+'WH1.SKU. sku='+@sku+', STORERkey='+@storerkey+'. Неудалось выполнить вставку новой записи.'+char(10)+char(13)
						set @send_error = 1
					end
					
					
					insert into wh2.sku(WHSEID,
						sku,storerkey,descr,packkey,rfdefaultpack,skugroup,--skugroup2,
						FREIGHTCLASS,
						--stdcube,stdgrosswgt, 
						BUSR1,BUSR2,
					...
					if @@rowcount = 0 
					begin
						set @msg_errdetails = @msg_errdetails+'WH2.SKU. sku='+@sku+', STORERkey='+@storerkey+'. Неудалось выполнить вставку новой записи.'+char(10)+char(13)
						set @send_error = 1
					end
				end
				...
				-----------Обработка заменяющего товара-----------------
					...
					begin						
						...
						if not exists (select 1 from wh2.altsku where sku = @sku and storerkey = @storerkey and altsku = @barcod)
						begin
						    print ' вставляем новые штрихкоды'
						    insert into wh2.altsku 
						    (whseid, storerkey, sku, altsku, packkey, defaultuom, [type], addwho)
						    select 'wh2' as whseid, 
							    @storerkey as storerkey, 
							    @sku as sku, 
							    @barcod as altsku,						     
							    'std' as packkey,		
