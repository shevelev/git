...
--################################################################################################
ALTER PROCEDURE [dbo].[app_Receipt]
...
print '...Извлекаем контрагентов'
	...
	from wh1.po po
		left join wh1.storer st on (po.sellersreference=st.storerkey)
...
select @status=status from wh1.receipt
...
if	((@Storer is not null) 
			or
		 (select Receiptkey from wh1.Receipt where Receiptkey=@Receiptkey) is not null )
	and
	(@Receiptkey<>'')
	and
	(ISNULL(@status,'0')='0')
...
if (select Receiptkey from wh1.Receipt where Receiptkey=@Receiptkey) is null
begin
