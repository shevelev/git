...
begin	
	...
	print 'сбрасываем статус таймаута (3 минуты) и пополнения (20 минут) , если он истек'
	update o set susr5 = ''--, o.EDITDATE=getdate()
	from wh1.orders o
		join #orders t on t.ORDERKEY = o.ORDERKEY 
	where t.susr5 in (@status_timeout, @status_repl) and dateadd(n, case when o.susr5 = @status_timeout then 3 else 20 end, o.editdate) <= getdate()
	
	update #orders set susr5 = ''--, EDITDATE=getdate() 
	where susr5 in (@status_timeout, @status_repl) and dateadd(n, case when susr5 = @status_timeout then 3 else 20 end, editdate) <= getdate()
...
--select SUSR5,* from #orders where susr5 in ( @status_wait, @status_repl)
			...
			if (dateadd(n,case when @susr5 = @status_wait then 3 else 20 end, @editdate) <= getdate())  
...
--		end -- while (@doNext and exists......
	...
	begin
		...
		from wh1.LOTXLOCXID lli 
			...
			---join #orderdetail od on  --od.LOTTABLE01 =  Шевелев С.С. 16.06.2015 (удаление задвоенных строк)
