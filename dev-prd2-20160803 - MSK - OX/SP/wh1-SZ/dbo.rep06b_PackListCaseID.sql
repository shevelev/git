/*¬ отчет rep06_PackListCaseID добавлена проверка на формировку отчета. —олдаткин*/
ALTER PROCEDURE [dbo].[rep06b_PackListCaseID] (
...
set @sql = '
'+case when isnull(@wave,'')<>'' and isnull(@order,'')='' and isnull(@caseid,'')=''
		then '	insert into adminsPRD1.dbo.Report_Quiry
				...
				left join '+@wh+'.orders ord on pd.orderkey=ord.orderkey
				left join '+@wh+'.wavedetail wd on ord.orderkey=wd.orderkey
				where ord.ORDERDATE between '''+@bdate+''' and '''+@edate+'''
						and wd.wavekey='''+@wave+'''
						and pd.CASEID not in(select caseid
											from adminsPRD1.dbo.Report_Quiry)
				order by CaseID	'
	else ''	
	end+'
'
...
--from adminsPRD1.dbo.Report_Quiry
	...
	FROM #sel SEL
		...
		left join adminsPRD1.dbo.Report_Quiry RQ on RQ.wave=wav.wavekey and RQ.CaseID=SEL.CASEID'
	+' where 1=1 '
...
update adminsPRD1.dbo.Report_Quiry
set	TypeDoc = 'ƒубликат'
