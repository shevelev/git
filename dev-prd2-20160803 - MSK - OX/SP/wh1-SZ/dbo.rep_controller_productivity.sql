ALTER PROCEDURE [dbo].[rep_controller_productivity]
...
---------------------------------------------------------
		...
		from wh1.PICKCONTROL pc with (NOLOCK,NOWAIT)
			join (select distinct ORDERKEY, ORDERLINENUMBER, CASEID, LOT from wh1.PICKDETAIL with (NOLOCK,NOWAIT)) c on c.CASEID = pc.CASEID and c.LOT = pc.LOT
		where pc.[STATUS] = '1'
