...
'select * FROM OPENROWSET(''MSDASQL'',''DSN=SMarket;UID=SYSINFOR;PWD=3'',
										''select 
											...
											round(ds.pricerub,2) unitprice,
											round(ts.tax1,2) nds,
											round(quantity,2) qty, 
											dh.post_status,
											dh.manager,
											dh.operation,
								...
								if (exists(
									select top(1) sdr.*
										...
										full join #smsumm ss on ss.receiptnumber = sdr.docnumber
										 ...
										 where sdr.docnumber = @id and ((round(isnull(wd.qty,0),2)-round(isnull(wd.qtybrack,0),2))-(isnull(sdr.qty,0)-isnull(ss.qty,0))) != 0))
									begin
							...
							if (exists(
									select top(1) sdr.docnumber
										...
										where sdr.docnumber = @id and (sdr.storerkey != wd.storerkey or 'SM'+sdr.storerkey2 != wd.storerkey2 
												or sdr.nds != round(wd.nds,2) or sdr.unitprice != round(wd.unitprice,2) or (round(wd.qty,2)-(sdr.qty-isnull(sdo.qty,0))) != 0) ))
									begin
										print ' в документах есть расхождения, удаляем их из выборки '+@id
										delete from sdr from #sm_docsr sdr where sdr.docnumber = @id
--										delete from sdo from #sm_docso sdo join #conformdocs cd on sdo.docnumber = cd.ordernumber where cd.receiptnumber = @id
			...
			-- ПЕРЕСЧЕТЫ - ПОЛНОЕ СООТВЕТСТВИЕ -----------------------------------------------------------------
				begin
					...
					select wmdocnumber, sku, zone, storerkey, sum(round(qty,2)) qty 
						from #sm_docsr
							...
							if (exists(
									select top(1) sdr.docnumber
										...
										where sdr.docnumber = @id and (sdr.storerkey = wd.storerkey and 'SM'+sdr.storerkey2 = wd.storerkey2 
												and sdr.nds = round(wd.nds,2) and sdr.unitprice = round(wd.unitprice,2) and (round(wd.qty,2)-(sdr.qty-isnull(sdo.qty,0))) = 0) ))
									begin
										print ' в документах нет расхождений, удаляем их из выборки '+@id
										delete from sdr from #sm_docsr sdr where sdr.docnumber = @id
--										delete from sdo from #sm_docso sdo join #conformdocs cd on sdo.docnumber = cd.ordernumber where cd.receiptnumber = @id
			...
			if @typedocument = 0
				begin 
					...
					print ' подготовка приходных документов инфор'
						insert into #wmsum
							select docnumber, smdocnumber, sum(round(qty,2)*round(unitprice,2)) wm_sum, null
