...
-- =============================================
ALTER PROCEDURE [WH1].[SZ_ABCRecalc] 
...
where pd.status='9' and (pd.editdate between getdate()-cast(WH1.GetFromCODELKUP('SYSVAR','ABCPERIOD') AS int) and getdate())
...
--Рассчитываем группу ABC
	...
	select @A=cast(isnull(LONG_VALUE,'0') as real) from wh1.CODELKUP where LISTNAME='ABC' and CODE='A'
	select @B=cast(isnull(LONG_VALUE,'0') as real) from wh1.CODELKUP where LISTNAME='ABC' and CODE='B'
	select @C=cast(isnull(LONG_VALUE,'0') as real) from wh1.CODELKUP where LISTNAME='ABC' and CODE='C'
...
update wh1.SKU
set sku.abc=S.ABC
from wh1.sku sku join #ship S on (sku.storerkey=S.storerkey and sku.sku=S.sku)
