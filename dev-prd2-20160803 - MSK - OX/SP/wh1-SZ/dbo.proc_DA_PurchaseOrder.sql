	...
	while (exists (select id from DA_PO))
		begin
			...
			begin
				...
				--	from #DA_PO r join wh1.storer s on  r.sellername = s.storerkey
				...
				--	from #DA_PO r join wh1.storer s on  r.STORERKEY = s.storerkey			
				...
				set
					...
					----packkey = case when (isnull(packkey,'') = '') then (select top(1) s.packkey from wh1.SKU s where s.SKU= sku and s.STORERKEY = STORERKEY) else ltrim(rtrim(packkey)) end,
