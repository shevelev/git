CREATE proc [rep].[Transactions_with_product]
...
--select @_sku, @_lot
	...
	from @TIMELINE t
		...
		join wh1.SKU s on s.STORERKEY = l.STORERKEY and s.SKU = l.SKU
		left join wh1.ADJUSTMENT a on t.TRANTYPE = 'AJ' and t.DOC_KEY = a.ADJUSTMENTKEY
--		left join wh1.RECEIPT r on t.TRANTYPE = 'DP' and t.DOC_KEY = r.RECEIPTKEY
		left join @PO po on t.TRANTYPE = 'DP' and po.RECEIPTKEY = t.DOC_KEY
--		left join wh1.TASKDETAIL t on t.TRANTYPE = 'MV' and t.SOURCEKEY = i.SOURCEKEY and i.SOURCEKEY > ''
		left join wh1.ORDERS o on t.TRANTYPE = 'WD' and t.DOC_KEY = o.ORDERKEY
		left join wh1.PICKDETAIL p
			left join wh1.TASKDETAIL td on td.PICKDETAILKEY = p.PICKDETAILKEY and td.LOT = p.LOT and td.CASEID = p.CASEID
	...
	;with tree (DROPID, CHILDID, THREAD, LEVEL) as (
		...
		from #TRANSACTIONS t
			join wh1.DROPIDDETAIL d on d.CHILDID = t.CASEID
		union all
