ALTER PROCEDURE [dbo].[proc_InventoryResult_old] (
		@wh varchar (10),
		@skugroup varchar (10),
		@hostzone varchar (20),
...
--			join '+@wh+'.hostzones h on h.putawayzone = l.putawayzone
		where t.adddate > '''+convert(varchar(19),@adddate,112)+' '+convert(varchar(19),@adddate,114)+''' 
			...
			and tablename = ''adjustment'' and t.eventcategory = ''E''
...
print 'удаление из выборки данных, не относяхися к складу'
	...
	-- создание таблицы
		select identity (int, 1,1) id, i.skugroup, i.storerkey, i.sku, sum(factqty) factqty, sum (deltaqty) deltaqty, i.sklad, s.descr into #result from #InventoryDetail i join wh1.sku  s on s.sku = i.sku and s.storerkey = i.storerkey and 2=1	group by i.skugroup, i.storerkey, i.sku, i.loc, i.sklad, s.descr
print 'подсчет дельты количества товара'
		...
		'insert into #result
			...
			from #InventoryDetail i join '+@wh+'.sku  s on s.sku = i.sku and s.storerkey = i.storerkey
