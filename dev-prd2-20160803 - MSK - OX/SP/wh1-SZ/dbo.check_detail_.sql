ALTER PROCEDURE [dbo].[check_detail_] (
	@dockey varchar (15),
...
-- определение системы документа
			...
			SELECT r_linenumber,r_articul,r_descr,sm_qty,sm_unitprice,sm_unitprice_1,sm_nds,sm_summs,sm_summs_1,0,0,0,0,'''',client_index,'''',name_clients,'''',sm_storerkey,''''
				FROM OPENROWSET(''MSDASQL'',''DSN=SMarket;UID=SYSINFOR;PWD=3'',
					...
					cs.name r_descr, 
					round(ds.quantity,2) sm_qty, 
					round(ds.pricerub,2) sm_unitprice, 
					round(ds.pricerub_1,2) sm_unitprice_1, 
					round(ts.tax1,2) sm_nds,
					round(ds.quantity,2)*round(ds.pricerub,2) sm_summs, 
					round(ds.quantity,2)*round(ds.pricerub_1,2) sm_summs_1,
					dh.client_index,
					cl.name_clients,
...
print @sql
			...
			select distinct 
				...
				r.sm_qty,
				round(r.sm_unitprice,2) sm_unitprice,
				round(r.sm_unitprice_1,2) sm_unitprice_1,
				round(r.sm_nds,2) sm_nds,
				round(r.sm_summ,2) sm_summ,
				round(r.sm_summ_1,2) sm_summ_1,
				round(r_r.wm_qty,2) wm_qty,
				round(r_r.wm_unitprice,2) wm_unitprice,
				round(r_r.wm_nds,2) wm_nds,
--				round(r_r.wm_summ,2) wm_summ,
				round(r_r.wm_qty,2)*round(r_r.wm_unitprice,2) wm_summ,
