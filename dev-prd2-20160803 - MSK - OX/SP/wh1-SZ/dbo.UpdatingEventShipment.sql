...
select top 1 o.ORDERKEY [Infor], o.EXTERNORDERKEY [Аксапта], o.ADDDATE [Загружен в Infor] ,max(oss.ADDDATE) [Отгружен из INfor], ax.createddatetime+'03:00' [Появился в DAX],ax.MODIFIEDDATETIME+'03:00' [Обработано DAX],
...
left join [SPB-SQL1210DBE\MSSQLDBE].[DAX2009_1].[dbo].SZ_ImpOutputOrdersShip ax on o.EXTERNORDERKEY=ax.docid
where o.STATUS in ('92','95') and o.EDITDATE > DATEADD(d, -4, getdate()) and ax.createddatetime is null
group by o.ORDERKEY, o.EXTERNORDERKEY,  o.ADDDATE, ax.createddatetime,ax.MODIFIEDDATETIME, ax.status, ax.error
order by 4 desc
