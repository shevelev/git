ALTER PROCEDURE [dbo].[rep01_ASNReceipt] (
/* 01 Акт приемки (ПУО) */
	@wh varchar(30),
	@asn varchar(15),
	@nav varchar(15),
...
BEGIN
	...
	set @sql = 'insert into #ASN (
		...
		FROM '+@wh+'.RECEIPTDETAIL AS RD 
			JOIN '+@wh+'.RECEIPT AS R ON R.RECEIPTKEY = RD.RECEIPTKEY 
			JOIN '+@wh+'.SKU AS S
				left join '+@wh+'.STORER mf on s.BUSR1 = mf.STORERKEY
			ON RD.SKU = S.SKU AND RD.STORERKEY = S.STORERKEY 
			JOIN '+@wh+'.PACK AS PK ON PK.PACKKEY = s.PACKKEY 
			JOIN '+@wh+'.STORER AS ST ON ST.STORERKEY = RD.STORERKEY 			
			LEFT JOIN '+@wh+'.PUTAWAYSTRATEGY as whpas on whpas.PUTAWAYSTRATEGYKEY = s.PUTAWAYSTRATEGYKEY
...
--	exec ('insert into #SKUERR select STORERKEY, SKU,'''' ERR from '+@wh+'.SKU')
--	exec ('insert into #SKUERR select s.STORERKEY, s.putawayzone, s.SKU, '''' ERR from '+@wh+'.SKU s right join #ASN
--			on (s.storerkey=#ASN.storerkey and s.sku=#ASN.sku)')

--	exec ('insert into #SKUERR select distinct SKU,'''' ERR from '+@wh+'.SKU')
...
select asn.*, bm.componentsku, asn.QTYEXPECTED*bm.qty compQTY, sk2.descr compname,
			...
			len(isnull(st.SUSR3,'')) LENSUSR, dbo.GetEAN128(asn.sku) EANSKU, rd.LOTTABLE02 lot2, rd.LOTTABLE04 lot4, rd.LOTTABLE05 lot5
		from #asn asn
			join wh1.RECEIPTDETAIL rd on rd.RECEIPTKEY=asn.RECEIPTKEY and rd.SKU=asn.sku --and rd.STATUS=0
...
group by asn.SUSR10 ,asn.STATUS, asn.RECEIPTKEY, asn.bcRECEIPKEY, asn.STORERKEY,	asn.externreceiptkey, asn.bcexternreceiptkey,
			asn.bcSTORERKEY, asn.COMPANY, asn.RECEIPTDATE, 	asn.SKU, asn.DESCR, asn.MANUFACTURER, asn.PACKKEY, asn.CASECNT, asn.PALLET, 
			asn.QTYEXPECTED, asn.QTYRECEIVED, asn.SCUBE, asn.Expr1, asn.CARRIERNAME, asn.CARRIERKEY, asn.SUSR1, asn.lottable07,		
			asn.WAREHOUSEREFERENCE,	asn.angar, asn.baseMeasure, asn.type, asn.lottable08,asn.bclottable08,asn.POKEY, asn.ER2, bm.componentsku, asn.QTYEXPECTED*bm.qty, sk2.descr,
			sumCaseReceived, sumReceived, isnull(sk2.susr4,'шт.') ,
			len(isnull(st.SUSR3,'')), dbo.GetEAN128(asn.sku) , rd.LOTTABLE02 , rd.LOTTABLE04 , rd.LOTTABLE05
	
	drop table #asn
	drop table #summary

--whpas.descr
--(SELECT COMPANY FROM  '+@wh+'.STORER WHERE      (STORERKEY = ''NORD'')) AS SLK, 
