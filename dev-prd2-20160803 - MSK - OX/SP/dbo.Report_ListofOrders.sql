ALTER PROCEDURE [dbo].[Report_ListofOrders] (
	@wh varchar(30),
...
--		set @date1 = dateadd(dy,-1,@date2)
	...
	select ord.ohtype, ck3.DESCRIPTION, ord.status, ord.orderkey, ord.externOrderkey, ord.storerkey, st.company StorerName, 
			...
			wd.wavekey Wave, ord.rfidflag, ord.adddate, st1.vat,ord.CarrierCode ,ord.CarrierName  --:
	into #orders
	from WH1.orders ord
...
--		case when isnull(@storer,'')='' then '' else ' AND st.company like '''+@storer+''' ' end +
		case when isnull(@storer,'')='' then '' else ' AND ord.storerkey like '''+@storer+''' ' end +
...
--select * from #orders
	...
	select ord.ohtype, ord.DESCRIPTION, ord.orderkey, externorderkey, storerkey, storername, clientcode, clientname, orderstatus,
		...
		PickWeight, PickVol, PickPalletsVavg, PickPallets, rfidflag, ord.adddate, ord.vat,ord.CarrierCode ,ord.CarrierName  --:
	into #result
	from #orders ord
