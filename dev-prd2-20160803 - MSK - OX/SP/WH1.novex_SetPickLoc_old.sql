...
--	...
-- =============================================
ALTER PROCEDURE [WH1].[novex_SetPickLoc_old] 
...
select @needLoc=wh1.novex_checkNeedSetPickLoc(@Storer,@SkuName)
...
--В дальнейшем, товары с подобным началом в названии не назначаются в одну ячейку.
	...
	select 			l.LOC,
					...
					''					LOCindex
	from wh1.loc l
		join WH1.PUTAWAYZONE Z on (l.PUTAWAYZONE=Z.PUTAWAYZONE)
		left join WH1.SKUXLOC sxl on (l.loc=sxl.loc)
		left join WH1.SKU s on (sxl.storerkey=s.storerkey and sxl.sku=s.sku)
	where 
	...
	set	SKUCOUNT=isnull((select sum(case when s.abc='A' then 2 when s.abc='D' then 0.5 else 1 end) 
					from wh1.skuxloc sxl join wh1.sku s on (sxl.storerkey=s.storerkey and sxl.sku=s.sku)
					where  sxl.loc=#processingLocs.loc and
					((sxl.allowreplenishfromcasepick=1 or sxl.allowreplenishfrombulk=1))
				  ),0)
	print '...Запоминаем, товар какой группы уже есть в данной ячейке'
	update #processingLocs
	set	ABC=isnull((select min(isnull(s.abc,'C')) from wh1.skuxloc sxl 
...
--select * from #processingLocs
	...
	insert into #PREselectedLocs
		select pl.LOC,pl.LOGICALLOCATION,pl.CUBICCAPACITY,pl.SKUCOUNT,pl.ABC,pl.loclevel,pl.SKUSTDCOUNT,
			...
			when 1 then
				case	
						--группа товара D (желательно: не менее 2х товаров в ячейке)
						--сортировка:  желательно ярус 3 [1-D/C/B/A,0-D/C/B/A]
						--сортировка:  ярус 1-2 [1-D/C/B/A, 2, (0 или 3), 4, 5, 6, 7, 8 ...]
						when isnull(@abc,'C')='D' then 
								case 
									when pl.loclevel=3 and  (pl.skucount>0    and pl.skucount<=1) then '000'
									when pl.loclevel<3 and  (pl.skucount>=0.5 and pl.skucount<=1) then '010'
						...
						--основная масса товара, группы A, B и С (желательно: 1 ячека=1 товар)
						--сортировка:  0-D/C/B/A, 1-D/C/B/A, 2-D/C/B/A, 3, 4 ...
						else
			...
			when 2 then  
				case	
						--группа товара A или B (желательно: 1 ячека=1 товар)
						--сортировка:  ярус 1-2 [0-D/C/B/A, 1-D/C/B/A, 2-D/C/B/A, 3, 4 ...]
						--сортировка:  ярус 3 и выше запрет размещения
						when isnull(@abc,'C')='A' or isnull(@abc,'C')='B' then 
							case 
							 when pl.loclevel<3 then
								replicate('0',3-len(cast(pl.skucount*10 as varchar(3)))) + cast(pl.skucount*10 as varchar(3))
								+ 
								(char(ascii('Z')-ascii(left(pl.abc,1))+ascii('A')))
							 else 
						...
						--группа товара D (желательно: не менее 3х товаров в ячейке)
						--сортировка:  желательно ярус 3 [1-D/C/B/A,0-D/C/B/A]
						--сортировка:  ярус 1-2 [2-D/C/B/A,1,(0 или 3), 4, 5, 6, 7, 8 ...]
						when isnull(@abc,'C')='D' then 
								case 
									when pl.loclevel=3 and  (pl.skucount>=0.5 and pl.skucount<=1) then '000'
									when pl.loclevel=3 and  pl.skucount=0 then '010'
									when pl.loclevel<3 and  pl.skucount=2 then '020'
									when pl.loclevel<3 and  (pl.skucount>=0.5 and pl.skucount<=1.5) then '030'
									when pl.loclevel<3 and  pl.skucount=0 then '040'
									else replicate('0',3-len(cast((pl.skucount+3+(case when pl.loclevel<3 then 0 else pl.loclevel-1 end))*10 as varchar(3)))) + cast((pl.skucount+3+(case when pl.loclevel<3 then 0 else pl.loclevel-1 end))*10 as varchar(3))
								end	
								+ (char(ascii('Z')-ascii(left(pl.abc,1))+ascii('A')))
						--основная масса товара, группа С (желательно: 1 ячека=2 товара)
						--сортировка:  ярус 1-2 [1-C/D/B/A, (0 или 2), 3, 4, 5 ...]
						else
								case pl.skucount+(case when pl.loclevel<3 then 0 else pl.loclevel-1 end)
									when 1   then '000'
									when 0.5 then '000'
									when 0 then '020'
									else replicate('0',3-len(cast((pl.skucount+1+(case when pl.loclevel<3 then 0 else pl.loclevel-1 end))*10 as varchar(3)))) + cast((pl.skucount+1+(case when pl.loclevel<3 then 0 else pl.loclevel-1 end))*10 as varchar(3))
								end	
								+
								case pl.abc
									when 'A' then 'Z'
			...
			--нормальное количество товаров в ячейке считаем равным 1 ячека=3 товара
				...
				case isnull(@abc,'C')
						when 'A' then --группа товара A (желательно: 1 ячека=1 товар)
								--сортировка:  ярус 1-2 [0-D/C/B/A, 1-D/C/B/A, 2-D/C/B/A, 3, 4 ...]
								--сортировка:  ярус 3 и выше запрет размещения
							case 
							 when pl.loclevel<3 then
						...
						when 'B' then --группа товара B (желательно: 1 ячека=2 товара)
								--сортировка:  ярус 1-2 [1-B/C/D/A, 2, (0 или 3), 4, 5 ...]
								--сортировка:  ярус 3 аналогично 1-2, но с ограничением на -2 позиции
								case pl.skucount+ (case when pl.loclevel<3 then 0 else pl.loclevel-1 end)
									when 0.5 then '000'
									when 1   then '000'
									when 1.5 then '010'
						...
						when 'D' then --группа товара D (желательно: не менее 4х товаров в ячейке)
								--сортировка:  желательно ярус 3 [1-D/C/B/A,0-D/C/B/A]
								--сортировка:  ярус 1-2 [3-D/C/B/A,2,1,4, (0 или 5), 6, 7, 8 ...]
								case 
									when pl.loclevel=3 and  (pl.skucount>=0.5 and pl.skucount<=1) then '000'
									when pl.loclevel=3 and  pl.skucount=0   then '010'
									when pl.loclevel<3 and  pl.skucount=1.5 then '020'
									when pl.loclevel<3 and  (pl.skucount>=0.5 and pl.skucount<=1) then '030'
									when pl.loclevel<3 and  pl.skucount=2 then '040'
									when pl.loclevel<3 and  pl.skucount=0 then '050'
									else replicate('0',3-len(cast((pl.skucount+5+(case when pl.loclevel<3 then 0 else pl.loclevel-1 end))*10 as varchar(3)))) + cast((pl.skucount+5+(case when pl.loclevel<3 then 0 else pl.loclevel-1 end))*10 as varchar(3))
								end	
								+ (char(ascii('Z')-ascii(left(pl.abc,1))+ascii('A')))
						else	--основная масса товара, группа С (желательно: 1 ячека=3 товара)
								--сортировка:  ярус 1-2 [2-C/D/B/A, 1, (0 или 3), 4, 5 ...]
								case pl.skucount+(case when pl.loclevel<3 then 0 else pl.loclevel-1 end)
									when 0.5 then '000'
									when 1   then '000'
									when 1.5 then '000'
		...
		where pl.loc not in	(
					...
					from #processingLocs pl2
						join wh1.skuxloc sxl on (pl2.loc=sxl.loc)
						join wh1.sku s on (sxl.storerkey=s.storerkey and sxl.sku=s.sku)
					where
...
--	truncate table #selectedLocs
	...
	select 'WH1',  @Storer,@SkuName,sl.LOC,		 'PICK',				 0,				  0,			 '2',						  1,				     1,					 '4','novex_SetPickLoc'
	from #selectedLocs sl
		left join wh1.skuxloc sxl on (sl.loc=sxl.loc and sxl.storerkey=@Storer and sxl.sku=@SkuName)
	...
	from wh1.skuxloc sxl join #selectedLocs sl   on (sl.loc=sxl.loc)
	...
	print '......товар новый, создаем пустую партию'
		...
		select	'WH1',	@newLot,@Storer,	@SkuName,	'novex_SetPickLoc',	'novex_SetPickLoc'
		insert into wh1.lot
			(	whseid,	lot,	storerkey,	sku,		addwho,				editwho)
		select	'WH1',	@newLot,@Storer,	@SkuName,	'novex_SetPickLoc',	'novex_SetPickLoc'
	...
	--select * from #selectedLocs
	select 'WH1' whseid, lld2.lot, sxl.loc, @Storer storerkey, @SkuName sku
	into #newLLD
	from #selectedLocs sxl
		left join wh1.lotxlocxid lld on (sxl.loc=lld.loc and lld.storerkey=@Storer and lld.sku=@SkuName)
