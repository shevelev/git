...
ALTER PROCEDURE [rep].[mof_SP_CASEID_AUTO_PRINT2] 
@caseid varchar(20)
AS
...
insert into wh2.caseid_label (caseid, label,box)
...
update wh2.caseid_label set [print] = @pole, label=[dbo].STR_1251_UTF8(label) where caseid=@caseid 
...
begin
	...
	set @result = 'bcp "SELECT top 1 label FROM prd2.wh2.caseid_label where caseid='+@caseid+' order by id desc" queryout e:\label\'+@name+' -c -T -C RAW'
	exec @r2 = master..xp_cmdshell @result

IF (@r2 = 0)
begin
   PRINT 'Генерим файл с количеством этикеток'
	set @result = 'bcp "SELECT top 1 [print] FROM prd2.wh2.caseid_label where caseid='+@caseid+' order by id desc" queryout e:\label\print.ftp -c -T -C RAW'
