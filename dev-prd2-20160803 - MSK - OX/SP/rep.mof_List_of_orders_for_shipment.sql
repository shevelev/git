...
--################################################################################################
ALTER PROCEDURE [rep].[mof_List_of_orders_for_shipment]
	@dateLow		datetime,
	@dateHigh		datetime,
...
select @status=max(cast(o.status as int)) from wh2.orders o
...
select @DriverName=isnull(company,'') from wh2.storer where storerkey=@Driver
select @CarNumber=isnull(vat,''), @CarType=isnull(company,'') from wh2.storer where storerkey=@Car
...
begin
		...
		update o set ordergroup=@newWave from wh2.orders o where o.orderkey=@Orderkey
...
begin
		...
		from wh2.orders o	join wh2.Wave w on (o.ordergroup=w.wavekey)	-- эти join нужны для проверки существования
							join wh2.Loadhdr l on (o.loadid=l.loadid)	-- соответствующих Волн и Загрузок
		where o.orderkey=@Orderkey
		print '...очищаем номер Волны в ЗО'
		update o
		set ordergroup='' from wh2.orders o	where o.orderkey=@Orderkey
...
begin
		...
		exec dbo.DA_GetNewKey 'wh2','WAVEKEY',@newWave output
...
o.editdate
into #preList
from wh2.orders o join wh2.orderdetail od on (o.orderkey=od.orderkey)
where
...
from #preList o
		join wh2.storer st on (o.storerkey=st.storerkey)
		left join wh2.storer st_c on (o.consigneekey=st_c.storerkey)
		join wh2.orderstatussetup os on (o.status=os.code)
		join wh2.sku s on (o.storerkey=s.storerkey and o.sku=s.sku)
		left join wh2.loadhdr [load] on (o.loadid=[load].loadid)
		LEFT join wh2.storer st2 on (o.CONSIGNEEKEY=st2.STORERKEY)
