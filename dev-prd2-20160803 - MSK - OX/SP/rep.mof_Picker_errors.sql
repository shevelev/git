...
*/
ALTER PROCEDURE [rep].[mof_Picker_errors]
		...
		from wh2.PICKCONTROL pc with (NOLOCK, NOWAIT)
			join (
				...
				from wh2.PICKDETAIL p with (NOLOCK, NOWAIT)
					join wh2.ITRN i with (NOLOCK,NOWAIT) on i.SOURCEKEY = p.PICKDETAILKEY
...
--select * from #PROBLEM_ROWS order by 1,2
	...
	select distinct
		...
		p.TOLOC
	from #PROBLEM_ROWS x
		join wh2.ORDERS o on o.ORDERKEY = x.ORDERKEY
		join wh2.SKU s on s.SKU = x.SKU and s.STORERKEY = '001'
		join wh2.ORDERDETAIL od with (NOLOCK,NOWAIT) on od.ORDERKEY = x.ORDERKEY and od.SKU = x.SKU -- возможны повторы!
		join (
			select
				p.ORDERKEY,
				p.SKU,
				...
				sum(i.QTY) as PICKED_QTY
			from wh2.PICKDETAIL p with (NOLOCK,NOWAIT)
				join wh2.ITRN i with (NOLOCK,NOWAIT)
		...
		left join (
			...
			from wh2.PICKCONTROL pc with (NOLOCK, NOWAIT)
				...
				join (
					...
					from #PROBLEM_ROWS x
						join wh2.PICKDETAIL pd with (NOLOCK, NOWAIT) on pd.ORDERKEY = x.ORDERKEY and pd.SKU = x.SKU
