ALTER PROCEDURE [dbo].[mof_rep06_PackListCaseID] (
...
--select @wh = 'wh2', @order='0000006898'--, @caseid='0000068802'
...
declare	@sql varchar (max),
	...
	into #altsku from wh2.altSKU group by storerkey,sku

	update a1
	set a1.altsku=a2.altsku
	from #altsku a1 
		join wh2.altsku a2 on a1.serialkey=a2.serialkey
	...
	into #Sel
	FROM wh2.PICKDETAIL AS P 
		LEFT JOIN wh2.STORER AS S ON P.STORERKEY = S.STORERKEY 
		LEFT JOIN wh2.SKU AS SK ON SK.STORERKEY = P.STORERKEY AND SK.SKU = P.SKU 
		LEFT JOIN #altsku as aSK on SK.SKU=aSK.sku and SK.Storerkey=aSK.Storerkey
		LEFT JOIN wh2.ORDERS AS O ON O.ORDERKEY = P.ORDERKEY 
		LEFT JOIN wh2.PACK AS PK ON PK.PACKKEY = P.PACKKEY 
		LEFT JOIN wh2.LOC AS L ON P.LOC = L.LOC 
		left join wh2.STORER as car on O.CarrierCode=car.STORERKEY
		LEFT JOIN wh2.STORER AS st1 ON st1.STORERKEY = O.CONSIGNEEKEY
		left join wh2.LOTATTRIBUTE la on la.sku = p.sku and la.STORERKEY = p.STORERKEY
	WHERE 1=2
