...
-- =============================================
ALTER PROCEDURE [WH2].[novex_TRANZITLIST] (
@fil varchar(15)
)
AS
...
from WH2.TRANSSHIP TR 
	left join WH2.storer st1 on (TR.CUSTOMERKEY=st1.storerkey)
	left join WH2.storer st2 on (TR.VENDORKEY=st2.storerkey)
	left join WH2.dropid mixID on (dbo.func_return_general_dropid_from_caseid(containerid)=mixID.dropid)
where TR.status=0 
