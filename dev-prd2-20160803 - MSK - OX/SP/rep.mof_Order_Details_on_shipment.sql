...
ALTER PROCEDURE [rep].[mof_Order_Details_on_shipment] (
	@WH varchar(30),
	@order varchar(20)
)
AS
...
--select @order = '0000006250', @WH='wh2'
	...
	into #ordDet
	 from wh2.orderdetail  od
		left join wh2.sku s on od.sku=s.sku and od.storerkey=s.storerkey
		left join wh2.orderstatussetup oss on oss.code = od.status
	...
	into #picks
	from wh2.pickdetail where 1=2 --orderkey = @order and status in (5,6)
