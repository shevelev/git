...
BEGIN TRY
	--while (exists (select id from DA_PO))
		begin
			...
			print ' выбираем запись из обменной таблицы DA_PurchaseOrderHead_OX'
			select top(1) *
			into #DA_PO 
			from dbo.DA_PO 
			where id = @id
		...
		set @msg_errdetails1 =''
			...
			select 
				...
				@msg_errdetails1 = @msg_errdetails1 --sellername in STORER
					+case when (not exists(select s.* from wh1.storer s where s.storerkey = r.sellername))
			...
			begin
				...
				--------------- Замениям владельца в шапке документа ----------- Шевелев 21.06.2016
				update	s
				...
				--------------- Проставляем владельцу тип 1 ----------- Шевелев 21.06.2016
				update	s
					set	s.type=1
					from	     wh1.storer s
...
------------------------------------------------------------------------------------------------------------			
				print 'Закачиваем товары и ШК в базу'					
...
--select * from wh1.sku where SKU='10005'

while (exists (select id from #sku_xo))
	begin
		select top 1 @id_xo=id, @storerkey_xo=storerkey, @sku_xo=Sku from #sku_xo
		print 'есть запись в обменной таблице по заказу' + @id_xo + ' : ' + @storerkey_xo
		
		print 'Проверяем есть ли такой товар + владелец в базе товаров по wh1.sku '
		
			if (exists (select ws.* from wh1.sku ws where ws.storerkey = @storerkey_xo and ws.sku = @sku_xo))
					begin
						print ' sku='+@sku_xo+' и STORERkey='+@storerkey_xo+' уже существует'
						print ' обновляем WH1.sku='+@sku_xo+', storer='+@storerkey_xo
					end
			else 
					begin
						print ' sku='+@sku_xo+' и STORERkey='+@storerkey_xo+' нет в базе'
						print ' Создаем новую карточку товара='+@sku_xo+' для владельца='+@storerkey_xo
							insert into wh1.sku ([WHSEID],[STORERKEY],[SKU],[HAZMATCODESKEY],[DESCR],[SUSR1],[SUSR2],[SUSR3],[SUSR4],[SUSR5]
							...
							from wh1.sku where SKU=@sku_xo and STORERKEY='001'
						 
						 
						 print 'Вставляем ШК для этого товара'
							insert into wh1.altsku    (whseid, storerkey, sku, altsku, packkey, defaultuom, [type], addwho)
							select whseid,  @storerkey_xo as storerkey, sku, altsku, packkey, defaultuom, [type], 'sku_xo' as addwho 
							from wh1.ALTSKU where SKU=@sku_xo and STORERKEY='001'
							  
					end
		
		delete #sku_xo where id=@id_xo
		print 'удаляем запись' + @id_xo
	end	
					...
					begin
						...
						select 
							...
							@msg_errdetails1 = @msg_errdetails1 --sku in SKU
								+case when (not exists(select s.* from wh1.sku s where s.storerkey = rd.storerkey and s.SKU = rd.sku))
									then 'er#014PO. EXTERNPOkey=*'+rd.externpokey+'*, EXTERNLINENO=*'+rd.externlinenumber+'*. SKU=*'+rd.sku+' === ' + rd.storerkey +' ===* отсутвует в справочнике SKU.'+@enter
									else ''
								end,
				...
				if @msg_errdetails = '' 
					begin						
					    ...
					    begin
							...
							if @@ERROR <> 0
							begin
								set @msg_errdetails = @msg_errdetails+'PO.externpokey='+@externpokey+'. Неудалось выполнить вставку записи (строки документа).'+char(10)+char(13)
								set @send_error = 1
							end
						end
					end
