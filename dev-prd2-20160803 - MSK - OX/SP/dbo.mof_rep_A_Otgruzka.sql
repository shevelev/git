ALTER PROCEDURE [dbo].[mof_rep_A_Otgruzka] (
    @ord varchar(10)
)
as
begin
...
select distinct pd.CASEID, pd.dropid into #orders from wh2.PICKDETAIL pd where ORDERKEY='0000233662'
...
while exists(select top 1 * from #orders)
	begin
		...
		insert into #rez
		select @caseid, childid, dropid,  addwho, adddate  from wh2.DROPIDDETAIL where CHILDID=@caseid
	...
	select  @caseid2=dropid, @dropid=childid from #rez 
		insert into #rez
	select @caseid, childid, dropid, addwho, adddate from wh2.DROPIDDETAIL where CHILDID=@caseid2 and dropid!=@dropid
	set @iLevel=@iLevel+1
