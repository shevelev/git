ALTER PROCEDURE [rep].[mof_Build_errors_Alternative]
	--@orderkey varchar(18) = NULL,
	

	@date_from datetime = null,--'10.08.2015',
...
begin
	...
	from wh2.ORDERDETAIL od
		join wh2.ORDERS o on o.ORDERKEY = od.ORDERKEY
		left join (
		...
		left join wh2.PICKCONTROL pc on pc.CASEID = p2.CASEID and pc.LOT = p2.LOT
	...
	--select * from wh2.ORDERSTATUSSETUP
	--select * from wh2.PICKDETAIL p join wh2.LOC l on l.LOC = p.LOC
	...
	from #PROBLEM_LINES pl
		join wh2.ORDERDETAIL od
			join wh2.ORDERS o on o.ORDERKEY = od.ORDERKEY
			join wh2.SKU n on n.SKU = od.SKU and n.STORERKEY = od.STORERKEY
		on od.ORDERKEY = pl.ORDERKEY and od.ORDERLINENUMBER = pl.ORDERLINENUMBER
		left join wh2.PICKDETAIL p on p.ORDERKEY = pl.ORDERKEY and p.ORDERLINENUMBER = pl.ORDERLINENUMBER and p.QTY <> 0
		left join wh2.TASKDETAIL t on t.PICKDETAILKEY = p.PICKDETAILKEY
		join wh2.CODELKUP s on p.[STATUS] = s.CODE and s.LISTNAME = 'ORDRSTATUS'
		left join wh2.PICKCONTROL pc on pc.CASEID = p.CASEID and pc.LOT = p.LOT
	...
	from wh2.ORDERDETAIL od
		join wh2.ORDERS o on o.ORDERKEY = od.ORDERKEY
		join wh2.PICKDETAIL p on p.ORDERKEY = od.ORDERKEY and p.ORDERLINENUMBER = od.ORDERLINENUMBER
		join wh2.PICKCONTROL pc on pc.CASEID = p.CASEID and pc.LOT = p.LOT
	where o.[STATUS] >= 78 -- проконтролированы
		and ( @date_from is NULL or od.EDITDATE >= @date_from )
		and ( @date_to is NULL or od.EDITDATE <= @date_to )
	...
	--select *
	--from #ERRORS
	--where ORDERKEY = '0000336440'
	
	select externorderkey,orderkey,'' orderlinenumber,sku,descr,SER,exp_date,sum(originalqty) originalqty,dropid,caseid,fromloc,SUM(picked_qty) picked_qty,Max(picked_date)picked_date,Picked_user,contrlled_qty,Max(contrlled_date)contrlled_date,contrlled_user
	from #ERRORS
	--where ORDERKEY = '0000336440'
	GROUP BY externorderkey,orderkey,sku,descr,SER,exp_date,dropid,caseid,fromloc,Picked_user,contrlled_qty,contrlled_user
